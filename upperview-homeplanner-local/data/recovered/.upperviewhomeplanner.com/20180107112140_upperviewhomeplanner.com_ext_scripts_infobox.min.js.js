...
"click",c),this.eventListener3_=google.maps.event.addDomListener(this.div_,"dblclick",c),this.eventListener4_=google.maps.event.addDomListener(this.div_,"mouseover",function(){this.style.cursor="default"})),this.contextListener_=google.maps.event.addDomListener(this.div_,"contextmenu",e),google.maps.event.trigger(this,"domready"))};
...
InfoBox.prototype.panBox_=function(a){var b=0,c=0;if(!a&&(a=this.getMap(),a instanceof google.maps.Map)){a.getBounds().contains(this.position_)||a.setCenter(this.position_);a.getBounds();var e=a.getDiv(),k=e.offsetWidth,e=e.offsetHeight,h=this.pixelOffset_.width,f=this.pixelOffset_.height,l=this.div_.offsetWidth,i=this.div_.offsetHeight,j=this.infoBoxClearance_.width,g=this.infoBoxClearance_.height,d=this.getProjection().fromLatLngToContainerPixel(this.position_);d.x<-h+j?b=d.x+h-j:d.x+l+h+j>k&&(b=
d.x+l+h+j-k);this.alignBottom_?d.y<-f+g+i?c=d.y+f-g-i:d.y+f+g>e&&(c=d.y+f+g-e):d.y<-f+g?c=d.y+f-g:d.y+i+f+g>e&&(c=d.y+i+f+g-e);0===b&&0===c||(a.getCenter(),a.panBy(b,c))}};
